...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.REPO_MANAGER_TEST_LIVE ||
    'TRUE' === process.env.REPO_MANAGER_TEST_OVERRIDE
  ) {
  ...
  m.REPO_MANAGER_TEST_EXPLAIN = process.env.REPO_MANAGER_TEST_EXPLAIN || m.REPO_MANAGER_TEST_EXPLAIN
