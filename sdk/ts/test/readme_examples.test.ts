...
const TEST_SEED = {"entity":{"inbox":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+RepoManagerSDK\b/.test(code) ||
    /\bRepoManagerSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RepoManagerSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RepoManagerSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
