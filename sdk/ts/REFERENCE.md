# RepoManager TypeScript SDK Reference

Complete API reference for the RepoManager TypeScript SDK.
...
```ts
new RepoManagerSDK(options?: object)
```
...
#### `RepoManagerSDK.test(testopts?, sdkopts?)`
...
**Returns:** `RepoManagerSDK` instance in test mode.
...
Return the parent `RepoManagerSDK` instance.
