...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("REPO_MANAGER_TEST_LIVE") == "TRUE" ||
		os.Getenv("REPO_MANAGER_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("REPO_MANAGER_TEST_EXPLAIN"); explain != "" {
		m["REPO_MANAGER_TEST_EXPLAIN"] = explain
	}
