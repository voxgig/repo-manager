...
func TestInboxEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Inbox(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Inbox(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		inboxRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.inbox")))
		...
		inboxRef01Match := map[string]any{}

		inboxRef01ListResult, err := inboxRef01Ent.List(inboxRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, inboxRef01ListOk := inboxRef01ListResult.([]any)
		if !inboxRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", inboxRef01ListResult)
		}
...
func inboxBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("REPO_MANAGER_TEST_INBOX_ENTID")
	...
	idmapResolved := core.ToMapAny(env["REPO_MANAGER_TEST_INBOX_ENTID"])
	if idmapResolved == nil {
		idmapResolved = core.ToMapAny(idmap)
	}

	if env["REPO_MANAGER_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewRepoManagerSDK(core.ToMapAny(mergedOpts))
	}
	...
	return &entityTestSetup{
		...
		idmap:         idmapResolved,
		env:           env,
		explain:       env["REPO_MANAGER_TEST_EXPLAIN"] == "TRUE",
		live:          live,
